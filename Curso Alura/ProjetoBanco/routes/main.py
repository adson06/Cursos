...
from models.user import LoginPayload
...
main_bp = Blueprint('main_dp', __name__)
...
def get_product_by_id(product_id):
    ...
    if product:
        ...
        return jsonify({"message":f"Produto não encontrado com o id {product_id}"})
...
def upload_sales(token):
    ...
    return jsonify({"error": "Formato de arquivo inválido. Envie um CSV."}), 400